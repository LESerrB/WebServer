...
static HTTP_IO_RESULT HTTPDeleteFiles(HTTP_CONN_HANDLE connHandle) {
    ...
    #define SM_POST_FILE_READ_NAME       (0u)
    #define SM_POST_FILE_READ_VALUE      (1u)
    ...
    {
        // Find the name
        case SM_POST_FILE_READ_NAME:
            // Read a name
            if(TCPIP_HTTP_PostNameRead(connHandle, httpDataBuff, httpBuffSize) == HTTP_READ_INCOMPLETE)
                return HTTP_IO_NEED_DATA;

            TCPIP_HTTP_CurrentConnectionPostSmSet(connHandle, SM_POST_FILE_READ_VALUE);
            // No break...continue reading value

        case SM_POST_FILE_READ_VALUE:

            // If value is expected, read it to data buffer,
            // otherwise ignore it (by reading to NULL)
            if(!strcmp((char *)httpDataBuff, (const char *)"delfile"))
                cDest = httpDataBuff;
            else
                cDest = NULL;
            ...
            // If this was an unexpected value, look for a new name
            if(!cDest){
                TCPIP_HTTP_CurrentConnectionPostSmSet(connHandle, SM_POST_FILE_READ_NAME);
                break;
            }
            else
                task = 3;
